...
from loss.SARB import BCELoss, ContrastiveLoss, PartialSupConLoss
...
def main():
    ...
    # ---------------------------------------------------------------------
    # Temperature (温度) 建议设为 0.07 或 0.1
    criterion = {
        'BCELoss': BCELoss(reduce=True, size_average=True).cuda(),
        'SupConLoss': PartialSupConLoss(temperature=args.supConTemp).cuda(),
    }
    # ---------------------------------------------------------------------
...
def Train(train_loader, model, criterion, optimizer, writer, epoch, args):
    ...
    for batchIndex, (sampleIndex, input, target, groundTruth) in enumerate(train_loader):
        ...
        with torch.autograd.set_detect_anomaly(True):
            # ----------------------------------------------------------------
            # 无论是否到达 mixupEpoch，我们都需要 target 传进去以生成 contrastive_features
            # 注意：这里需要配合上一轮修改后的 model.forward 返回值顺序
            # 假设顺序是: result, semanticFeature, mixedResult_1, mixedTarget_1, mixedResult_2, mixedTarget_2, contrastive_features

            output_tuple = model(input, target=target)

            # 解包基础输出
            output = output_tuple[0]
            feature = output_tuple[1]

            # [关键] 获取对比特征 (假设它在 tuple 的最后一个位置)
            contrastive_features = output_tuple[-1]

            # 解包 Mixup 输出 (如果存在)
            # 即使 epoch < mixupEpoch，因为传入了 target，模型可能也会计算 mixup，
            # 但我们在 loss 计算时通过 mask (epoch判断) 来忽略它们即可。
            if len(output_tuple) > 3:
                mixedOutput1 = output_tuple[2]
                mixedTarget1 = output_tuple[3]
                mixedOutput2 = output_tuple[4]
                mixedTarget2 = output_tuple[5]
            else:
                # 兼容性处理，防止 model 未修改完全报错
                mixedOutput1, mixedTarget1, mixedOutput2, mixedTarget2 = None, None, None, None

        # Compute and log loss
        loss1_ = criterion['BCELoss'](output, target)

        # -------------------------------------------------------------------
        # SupCon Loss (替代原来的 loss2_)
        # 使用 contrastive_features 而不是 raw feature
        # 权重建议：SupCon 值较大，建议 args.contrastiveLossWeight 设为 0.1 或 0.05
        # 保留了原有的 warmup 策略 (第1个epoch权重线性增加)
        if epoch >= 1:
            loss2_ = args.contrastiveLossWeight * criterion['SupConLoss'](contrastive_features, target)
        else:
            loss2_ = args.contrastiveLossWeight * criterion['SupConLoss'](contrastive_features,
                                                                          target) * batchIndex / float(
                len(train_loader))

        # Mixup Losses (保持逻辑：mixupEpoch 之前为 0)
        if epoch < args.mixupEpoch:
            loss3_ = 0 * loss1_  # 保持梯度图连接但值为0
            loss4_ = 0 * loss1_
        else:
            loss3_ = criterion['BCELoss'](mixedOutput1, mixedTarget1)
            loss4_ = criterion['BCELoss'](mixedOutput2, mixedTarget2)
        ...
        if batchIndex % args.printFreq == 0:
            logger.info(
                '[Train] [Epoch {0}]: [{1:04d}/{2}] Batch Time {batch_time.avg:.3f} Data Time {data_time.avg:.3f} Learn Rate {lr:.6f} alpha {alpha:.4f} beta {beta:.4f}\n'
                '\t\t\t\t\tBCE Loss {loss1.avg:.4f} Contrastive Loss {loss2.avg:.4f} Instance Loss {loss3.avg:.4f} Prototype Loss {loss4.avg:.4f}'.format(
                    ...
                    lr=optimizer.param_groups[0]['lr'], alpha=model.alpha.data.item(), beta=model.beta.data.item(),
